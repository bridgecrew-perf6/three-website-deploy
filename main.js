...
//オブジェクトを作成
const boxGeometry = new THREE.BoxGeometry(5, 5, 5, 5, 10);
const boxMaterial = new THREE.MeshNormalMaterial();
const box = new THREE.Mesh(boxGeometry, boxMaterial);
box.position.set(0, 0.5, -15);//カメラの前に移動
box.rotation.set(1, 1, 0);

const torusGeometry = new THREE.TorusGeometry(8, 2, 16, 100);
const torusMaterial = new THREE.MeshNormalMaterial();
const torus = new THREE.Mesh(torusGeometry, torusMaterial);
torus.position.set(0, 1, 10);//カメラの後ろに移動

scene.add(box, torus);

//線形補間でなめらかに変化（A地点からB地点へなめらかに移動させる）
function lerp(x, y, a) {
  return (1 - a) * x + a * y;
}

function scalePercent(start, end) {
  //スクロール率におうじて各区間のどの位置にいるのか割合を算出
...
animationScripts.push({
  ...
  function() {
    ...
    //    box.position.z += 0.01;
    box.position.z = lerp(-15, 2, scalePercent(0, 40));//-15は初期値と同じものを入れる
...
document.body.onscroll = () => {
  //console.log("scroll");
  //(scrollX / (windowHeight-clientHeight))*100 = スクロール率
  let scrollX = document.documentElement.scrollTop;
  let windowHeight = document.documentElement.scrollHeight;
  let clientHeight = document.documentElement.clientHeight;
  scrollParcent = (scrollX / (windowHeight - clientHeight)) * 100;
